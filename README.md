# denoiser

A CLI tool for removing background audio noise from video files, delivering
cleaner sound without a complex setup.

## Features

- Removes background noise and hiss from video audio tracks
- Simple command-line interface for fast, scriptable workflows
- Supports common video formats MP4(.mp4), AVI(.avi), MOV(.mov),
  MKV(.mkv), WMV(.wmv), FLV(.flv), MPEG(.mpeg), WebM(.webm)


## Requirements

- Python 3.9+
- ffmpeg
- MoviePy

## Installation

```bash
pip install denoiser
```

## Usage

```bash
denoiser --help
```

## Development

See CONTRIBUTING.md for setup and contribution guidelines.

## Acknowledgements
- MoviePy — video processing library used under the hood

## License

Michael George - MIT
